...
class VisionTestBench(object):
  ...
  def train(self, trainingVectors, trainingTags, classifier, maxCycles=10, minAccuracy=100.0):
    """
    ...
    while (minAccuracy - accuracy) > 1.0/len(trainingTags) and cyclesCompleted < maxCycles:
      ...
      accuracy = 0.0
      testVectors,testingTags  = dataset_readers.randomiseImagesAndTags(trainingVectors, trainingTags)
      for j,testVector in enumerate(testVectors):
        self.sp.compute(testVector, False, activeArray)
        # if there are repeat tags give the index of the first occurrence
        category = self.tags.index(testingTags[j])
        inferences = classifier.infer(activeArray)
        if inferences is not None:        
          inferred_category = inferences[0]
        else:
  ...
  def test(self, vectors, tags, classifier, verbose=0, learn=True):
    """
    This routine tests the spatial pooler on the bit vectors produced from the
    testing images.
    """
    if learn:
      print("\nTesting with continuous learning enabled:")
    else:
      print("\nTesting, no learning:")
    ...
    testVectors,testingTags  = dataset_readers.randomiseImagesAndTags(vectors, tags)
  ...
  def showPermsAndConns(self):
    ...
    pAndCImage.show(title='Permanences and Connections')


  def showPermanences(self):
    if self.permanencesImage == None:
      self.calcPermsAndConns()
    self.permanencesImage.show(title='Permanences')


  def showConnections(self):
    if self.connectionsImage == None:
      self.calcPermsAndConns()
    self.connectionsImage.show(title='Connections')
